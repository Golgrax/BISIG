# skeleton_service.py - Handles 3D landmark processing and interpolation
...
NEUTRAL_POSE_PATH = os.path.join(BASE_DIR, "neutral_pose.json")
    ...
    with vision.PoseLandmarker.create_from_options(opt_pose) as pose_l, \
        ...
        while cap.isOpened():
            ...
            if p_res.pose_landmarks:
                f_lms["pose"] = [{"x": round(lm.x, 4), "y": round(lm.y, 4), "z": round(lm.z, 4), "visibility": round(lm.visibility, 4)} for lm in p_res.pose_landmarks[0]]
            
            if f_res.face_landmarks:
                f_lms["face"] = [{"x": round(lm.x, 4), "y": round(lm.y, 4), "z": round(lm.z, 4)} for lm in f_res.face_landmarks[0]]
            ...
            if h_res.hand_landmarks and f_lms["pose"]:
                ...
                for i, h_pts in enumerate(h_res.hand_landmarks):
                    ...
                    f_lms[key] = [{"x": round(lm.x, 4), "y": round(lm.y, 4), "z": round(lm.z, 4)} for lm in h_pts]
...
def render_skeleton_video(skeleton_data, output_path, width=640, height=480, fps=30):
    ...
    Using VP8/WebM for native browser playback.
    """
    ...
    for frame in skeleton_data:
        ...
        if frame.get("pose"):
            ...
            active_indices = {11, 12, 13, 14, 15, 16, 23, 24, 25, 26, 27, 28}
            for i, lm in enumerate(lms):
                if i in active_indices and lm.get("visibility", 1.0) > 0.5:
        ...
        for key in ["left_hand", "right_hand"]:
            if frame.get(key):
                lms = frame[key]
                # Check for collapsed hand (stabilization artifact)
                if abs(lms[4]["x"] - lms[20]["x"]) < 0.01 and abs(lms[4]["y"] - lms[20]["y"]) < 0.01:
                    continue
...
_NEUTRAL_POSE_CACHE = None

def GET_NEUTRAL_POSE():
    """
    Returns the hardcoded neutral/idle pole, loading it from JSON if necessary.
    """
    global _NEUTRAL_POSE_CACHE
    if _NEUTRAL_POSE_CACHE is None:
        if os.path.exists(NEUTRAL_POSE_PATH):
            with open(NEUTRAL_POSE_PATH, 'r') as f:
                _NEUTRAL_POSE_CACHE = json.load(f)
        else:
            # Fallback if file missing
            _NEUTRAL_POSE_CACHE = {"pose": None, "left_hand": None, "right_hand": None, "face": None}
    return _NEUTRAL_POSE_CACHE



def stabilize_skeleton(frames):
    """
    Ensures body parts don't disappear mid-animation.
    If a hand is missing, it anchors it to the wrist to prevent flickering.
    """
    ...
    for i in range(len(frames)):
        ...
        # Stabilize Hands: If hand is null but pose exists, anchor to wrist
        if frame.get("pose"):
            p = frame["pose"]
            for key, wrist_idx in [("left_hand", LEFT_WRIST), ("right_hand", RIGHT_WRIST)]:
                if not frame.get(key):
                    wrist = p[wrist_idx]
                    # Create a "collapsed" hand at the wrist position
                    frame[key] = [{"x": wrist["x"], "y": wrist["y"], "z": wrist.get("z", 0)} for _ in range(21)]
        
        # Stabilize Face: If face is null, try to keep previous face
        if not frame.get("face") and i > 0 and frames[i-1].get("face"):
            frame["face"] = frames[i-1]["face"]
